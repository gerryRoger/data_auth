...
public class OracleDialect extends AbstractHelperDialect{
	 ...
	 *  统筹区权限
	 */
//	public  final String DATA_SCOPE_AREA_SQL=" ${ALIAS_TABLE_NAME}.${AREA_COLUMN}='${%s}' ";
	 ...
	 */
//	public  final String DATA_SCOPE_CUSTOM_SQL=" ${ALIAS_TABLE_NAME}.${%s} IN ( SELECT ${%s} FROM ${%s} WHERE ${%s} = '${%s}' ) ";
	 ...
	 */
//	public  final String DATA_SCOPE_DEPT_SQL="  ${ALIAS_TABLE_NAME}.${%s}='${%s}'  ";
	public  final String DATA_SCOPE_DEPT_SQL="  ${ALIAS_TABLE_NAME}.${%s}= ?  ";
	
	/**CHECK_STATE
	 * 部门及以下数据权限 select dept_id from sys_dept start with dept_id='110'  connect by prior dept_id=dept_up_id
	 * ${ALIAS_TABLE_NAME}.${DEPT_ID_COL} in (select ${DEPT_ID_COL} from ${DEPT_TABLE_NAME} start with ${DEPT_ID_COL}='${DEPT_ID}'  connect by prior ${DEPT_ID_COL}=${DEPT_UP_ID_COL})
	 */
//	public  final String DATA_SCOPE_DEPT_AND_CHILD_SQL=" ${ALIAS_TABLE_NAME}.${%s} in ( select ${%s} from ${%s}  start with ${%s}='${%s}'  connect by prior ${%s}=${%s} )";
	public  final String DATA_SCOPE_DEPT_AND_CHILD_SQL=" ${ALIAS_TABLE_NAME}.${%s} in ( select ${%s} from ${%s}  start with ${%s}= ?  connect by prior ${%s}=${%s} )";
	
	/**
	 * 仅本人数据权限 ${ALIAS_TABLE_NAME}.${USER_ID_COLUMN}='${CRTER}'
	 */
//	public  final String DATA_SCOPE_SELF_SQL=" ${ALIAS_TABLE_NAME}.${%s}='${%s}' ";
	public  final String DATA_SCOPE_SELF_SQL=" ${ALIAS_TABLE_NAME}.${%s}=? ";
	/**
	 * 完全匹配 ${ALIAS_TABLE_NAME}.${COLUMN}='${value1}'
	 */
//	public  final String DATA_SCOPE_FULL_MATCH_SQL=" ${ALIAS_TABLE_NAME}.%s='%s' ";
...
//	public  final String DATA_SCOPE_CONTAIN_MATCH_SQL=" ${ALIAS_TABLE_NAME}.%s IN ( %s ) ";
	public  final String DATA_SCOPE_CONTAIN_MATCH_SQL=" ${ALIAS_TABLE_NAME}.%s IN ( ? ) ";
...
//			${ALIAS_TABLE_NAME}.${COLUMN}='${VALUE1}'
	...
	public String builderDataScopeContainMatch(String sql, Map<String, Object> param) {
		...
		try {
			...
			PlainSelect plain = (PlainSelect) select.getSelectBody();
			Table fromItem = (Table) plain.getFromItem();
			String containMatchColumn =  String.valueOf(param.get(FULL_MATCH_COLUMN));
			// 如果没有配置值，直接过
			...
			StringBuffer paramSql= new StringBuffer();
			for(int i=0,len=columns.length;i<len;i++){
				paramSql.append(String.format(DATA_SCOPE_CONTAIN_MATCH_SQL,columns[i],param.get(columns[i])));
				DataAuthHelper.getLocalList().add(new DataAuthParamValue(columns[i], param.get(columns[i])));
				if(i< len-1) {
					paramSql.append(" and  ");
				}
